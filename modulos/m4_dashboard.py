...
def ejecutar():
    ...
    # =================================================================
    # 📥 EXTRACCIÓN MASIVA (SIN LÍMITES) Y LIMPIEZA ANTI-CLONES
    # =================================================================
    with st.spinner("Sincronizando búnker de datos y cruzando telemetría..."):
        try:
            # 1. Extracción profunda de Estudiantes (Bucle while True)
            estudiantes_data = []
            ...
            # --- LA PURGA: Eliminar duplicados antes de cruzar ---
            if not df_estudiantes.empty:
                df_estudiantes = df_estudiantes.drop_duplicates(subset=["ID_Estudiante"]).copy()
                
            # 2. Extracción profunda de Notas (Bucle while True)
            notas_data = []
            offset, chunk_size = 0, 1000
            while True:
                res_not = supabase.table("notas_consolidadas").select("ID_Est, ASIGNATURA, P1, P2").range(offset, offset + chunk_size - 1).execute()
                if not res_not.data: break
                notas_data.extend(res_not.data)
                if len(res_not.data) < chunk_size: break
                offset += chunk_size
                
            df_notas = pd.DataFrame(notas_data)

        except Exception as e:
            st.error(f"🚨 Error de lectura masiva en la base de datos: {e}")
            return
